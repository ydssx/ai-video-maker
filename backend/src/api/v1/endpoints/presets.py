...
PRESETS_FILE = str((__import__('pathlib').Path(settings.output_path).parent / "presets.json"))
...
def ensure_presets_file():
    """确保预设文件存在"""
    os.makedirs(str(__import__('pathlib').Path(settings.output_path).parent), exist_ok=True)
