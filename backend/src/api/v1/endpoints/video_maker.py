...
async def create_video_internal(request: VideoRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # 若启用 Celery，提交异步任务；否则回退到本地后台任务
        if is_celery_enabled():
            # 使用 video_id 作为 Celery 任务ID，便于统一追踪
            create_video_task.apply_async(args=[video_id, script_dict, config], task_id=video_id)
        else:
...
async def get_video_status(video_id: str):
    ...
    try:
        ...
        # 获取处理状态（优先从 Celery 获取进度）
        progress_info = {"progress": 0}
        try:
            if is_celery_enabled():
                p = get_task_progress(video_id)
                progress_info = {"progress": p.get("progress", 0), "state": p.get("state")}
        except Exception:
            pass
        ...
        return {
            ...
            "progress": progress_info.get("progress", processing_status.get('progress', 0)),
