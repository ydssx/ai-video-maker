...
def test_database_service(mysql_url):
    """测试数据库服务"""
    try:
        # 测试模型定义
...
def main():
    ...
    print("测试数据库服务...")
    mysql_url = f"mysql+pymysql://{user}:{password}@{host}:{port}/{database}"
    if not test_database_service(mysql_url):
        return 1
    
    print()
    print("[SUCCESS] MySQL数据库设置完成！")
    print()
    print("接下来你可以:")
